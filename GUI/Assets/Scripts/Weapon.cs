...
public class Weapon : MonoBehaviour {
    ...
    [HideInInspector]
    public int bulletsLeft = 0;//弹夹中剩余子弹量
    [HideInInspector]
    public int bulletsReloaded = 0;//number of bullets reloaded during this reloading cycle
    [HideInInspector]
    public int ammo = 150;//储备子弹量，可以手动设置
    public int maxAmmo = 999;//最大子弹储备量
    ...
    void Start()
    {
        ...
        if (weaponsInBags.haveIndexOfWeapon(weaponNumber))//武器背包中持有对应编号的武器
        {
            
            myTransform = transform;//cache transform for efficiency
            mainCamTransform = Camera.main.transform;

            if (meleeSwingDelay == 0)//枪械
            ...
            weaponMeshAnimation.GetComponent<Animation>()["Fire"].speed = fireAnimSpeed;//initialize weapon mesh animation speeds
            weaponMeshAnimation.GetComponent<Animation>()["Reload"].speed = reloadAnimSpeed;
            weaponMeshAnimation.GetComponent<Animation>()["Ready"].speed = readyAnimSpeed;
            ...
            {//部分枪械有此动画
                weaponMeshAnimation.GetComponent<Animation>()["Neutral"].speed = 1.5f;
            }
        ...
        if (!weaponsInBags.haveIndexOfWeapon(weaponNumber) || meleeSwingDelay > 0)
        {
            canZoom = false;
        }
    ...
    void OnEnable()
    {
        ...
        weaponsBagObj = GameObject.FindGameObjectWithTag("WeaponBag");
        weaponsInBags = weaponsBagObj.GetComponent<WeaponsInBags>();
        //define external script reference
    ...
    void FixedUpdate()
    {
        ...
        {//allow pausing by setting timescale to 0
            ...
            ShowShellAmount ShellAmountGUI= weaponsBagObj.GetComponent<WeaponsInBags>().ammoGUI
                .GetComponent<ShowShellAmount>();//得到子弹容量GUI上的脚本
            ...
            AudioSource[] aSources = weaponsBagObj.GetComponents<AudioSource>();//Set up reference to Audio Sources using aSources array
            ...
            if (meleeSwingDelay == 0 && weaponsInBags.haveIndexOfWeapon(weaponNumber))
            {
                ////pass ammo amount to Gui object to be rendered on screen
                ShellAmountGUI.nowShellAmount = bulletsLeft;//main color
                ShellAmountGUI.backShellAmount = ammo;
            }
            else if (meleeSwingDelay != 0)
            {
                ShellAmountGUI.nowShellAmount = 0;//main color
                ShellAmountGUI.backShellAmount =0;
            }
            //do not perform weapon actions if this is an unarmed/null weapon
            if (weaponsInBags.haveIndexOfWeapon(weaponNumber))
            {
                ...
                if (!playerControl.getDead())
                {
                    ...
                    && !Input.GetKeyDown(playerControl.fire)
                    ...
                    {//cancel reload if player is climbing, swimming, or holding object and weapon is lowered
                        if (shootSight.reloading)
                        {
                            ...
                            if (bulletsToReload != bulletsPerClip)
                            {
                                ...
                                weaponMeshAnimation.GetComponent<Animation>()["Neutral"].speed = 1.5f;
                                weaponMeshAnimation.GetComponent<Animation>().Play("Neutral", PlayMode.StopAll);//play Neutral animation	
                            }
                            ...
                            {//if we are less than half way through reload before sprint interrupted, cancel reload
                             //stop reload sound from playing
                                ...
                                if (bulletsToReload == bulletsPerClip)
                                {
                                    ...
                                    weaponMeshAnimation.GetComponent<Animation>()["Reload"].speed = -reloadAnimSpeed * 1.5f;
                                    weaponMeshAnimation.GetComponent<Animation>().CrossFade("Reload", 0.35f, PlayMode.StopAll);//play reloading animation
                                }
                            }
                        }
                    }
                    ...
                    if (Input.GetKey(playerControl.reload)
                    && !shootSight.reloading
                    && ammo > 0
                    && bulletsLeft < bulletsPerClip
                    && shootStartTime + fireRate < Time.time
                    && !Input.GetKey(playerControl.fire))
                    {
                    ...
                    {//dont shoot if player is climbing
                        ...
                        {
                            //sprinting
                            if (((Input.GetKey(playerControl.moveForward) || Input.GetKey(playerControl.moveBack)))
                            || (Mathf.Abs(FPSWalkerComponent.inputY) > 0.1f))
                            {
                                sprintState = false;
                                if (shootSight.reloading)
                                {
                                ...
                                {
                                    if (playerControl.zoomed && meleeSwingDelay == 0)
                                    {
                                        canShoot = true;
                                    }
                                    else
                    ...
                    if (Input.GetKey(playerControl.fire))
                    {
                        if ((cantFireState)
                        && (canShoot)
                        && (bulletsLeft <= 0)
                    ...
                    //Change fire mode
                    if (Input.GetKey(playerControl.fireMode))
                    {
                        if (fireModeState
                        && canShoot
                        && !shootSight.reloading
                    ...
                    if (barrelSmokePos)
                    {
                        if (shootStartTime + 0.25f > Time.time)
                        {
                            if (barrelSmokeParticles && bulletsJustFired >= barrelSmokeShots)
                            {
                                ...
                                || ((shootStartTime + fireRate < Time.time) && Input.GetKey(playerControl.fire)))
                                {
                    ...
                    else
                    {
                        ...
                        {//animate weapon down
                         //store time that sprint anim started to disable weapon switching during transition
                         weaponsInBags.sprintSwitchTime = Time.time;
                            ...
                            if (weaponsInBags.switching&& shootSight.reloading)
                            {
                                weaponMeshAnimation.GetComponent<Animation>().CrossFade("Reload", 0.35f, PlayMode.StopAll);
                            }
                        }
                    }
                }
    ...
    {
        PlayerControl playerControl = playerObj.GetComponent<PlayerControl>();
        AudioSource[] aSources = weaponsBagObj.GetComponents<AudioSource>();//Initialize audio source
        ...
        {//allow pausing by setting timescale to 0
         //sync weapon sound effect pitch with timescale/bullet time
            ...
            //do not perform weapon actions if this is an unarmed/null weapon
            if (weaponsInBags.haveIndexOfWeapon(weaponNumber) && !playerControl.getDead())
            {
                ...
                if (Input.GetKey(playerControl.fire) && !waterFireState)
                {
                    if (semiAuto)
                    {
                        if (!semiState)
            ...
            //if (Time.deltaTime > 0.0f && weaponsInBags.haveIndexOfWeapon(weaponNumber))
            //{
    ...
    IEnumerator SpawnShell()
    {
        ...
        //shellEjectPosition = shellPrefabRB.GetComponent<GunShellScript>().shellPosition;
        ...
        GunShellScript gunShell = shell.GetComponent<GunShellScript>();
        //Initialize object references for instantiated shell object

        //对子弹上脚本变量进行初始化
        gunShell.playerObj = playerObj;
        gunShell.shellPosition = shell2.transform;//pass reference of mesh shell object to RB shell object's ShellEjection.cs component
        gunShell.gunObj = transform.gameObject;
    ...
    void Fire()
    {
        ...
        FPSRigidBodyWalker FPSWalkerComponent = playerObj.GetComponent<FPSRigidBodyWalker>();
        AudioSource[] aSources = weaponsBagObj.GetComponents<AudioSource>();
        AudioSource otherfx = aSources[1] as AudioSource;
        ...
        if ((bulletsToReload == bulletsPerClip && !shootSight.reloading)
        //allow normal firing when weapon does not reload by magazine
        || (!shootSight.reloading && bulletsToReload != bulletsPerClip && bulletsLeft > 0)
        //allow fire button to interrupt a non-magazine reload if there are at least 2 shells loaded
        || (shootSight.reloading && bulletsToReload != bulletsPerClip && bulletsLeft >= bulletsToReload * 2.0f && reloadEndTime + reloadTime < Time.time))
        {
            if (canShoot && !weaponsInBags.switching)
    ...
    IEnumerator FireOneShot()
    {
        ...
        if (canShoot)
        {
            ...
            firefx.PlayOneShot(firefx.clip, 0.9f / firefx.volume);//播放射击音效
            ...
            {//if this is not a melee weapon
                ...
                weaponMeshAnimation.GetComponent<Animation>().Rewind("Fire");
                weaponMeshAnimation.GetComponent<Animation>()["Fire"].speed = fireAnimSpeed;
                weaponMeshAnimation.GetComponent<Animation>().CrossFade("Fire", 0.35f, PlayMode.StopAll);//play firing animation

                bulletsLeft -= 1;//弹夹中子弹数减1	
                playerObj.GetComponent<PlayerControl>().point -= 20;//击发一颗子弹扣20分
            }
            else
            {

                if (swingSide)
                {//determine which side to swing melee weapon
                    ...
                    weaponMeshAnimation.GetComponent<Animation>().Rewind("MeleeSwingRight");
                    weaponMeshAnimation.GetComponent<Animation>()["MeleeSwingRight"].speed = fireAnimSpeed;//set weapon swing animation speed
                    weaponMeshAnimation.GetComponent<Animation>().CrossFade("MeleeSwingRight", 0.1f, PlayMode.StopAll);//play weapon swing animation
                ...
                else
                {
                    ...
                    weaponMeshAnimation.GetComponent<Animation>().Rewind("MeleeSwingLeft");
                    weaponMeshAnimation.GetComponent<Animation>()["MeleeSwingLeft"].speed = fireAnimSpeed;//set weapon swing animation speed
                    weaponMeshAnimation.GetComponent<Animation>().CrossFade("MeleeSwingLeft", 0.1f, PlayMode.StopAll);//play weapon swing animation
            ...
            for (float i = 0; i < projectileCount; i++)
            {
                ...
                else
                {
                    ...
                    if (Physics.SphereCast(Camera.main.transform.position, capsule.radius / 3, direction, out hit, range + (range * FPSWalkerComponent.playerHeightMod / 2.0f), bulletMask))
                    {
                        HitObject(hit, direction);
                        //if sphereCast hits nothing, try a rayCast to hit trigger colliders like the surface of water
                    }
                    else if (Physics.Raycast(Camera.main.transform.position, direction, out hit, range, bulletMask))
                    {
                        HitObject(hit, direction);
                    }
                }
    ...
    void HitObject(RaycastHit hit, Vector3 direction)
    {
        ...
        {//根据层数来区分打中的是什么
            //具体用来区分打中的是什么，除了施加力之外还要施加伤害
            case 13://打中僵尸
                if (hit.collider.tag == "zombieHead")
                {//打中僵尸头,两倍伤害
                    playerObj.GetComponent<PlayerControl>().point += 300;
                    hit.collider.gameObject.GetComponentInParent<AttackedByPlayer>().HitZombie(damage * 2, direction, mainCamTransform.position);
                }
                else
                {//打中僵尸身体
                    playerObj.GetComponent<PlayerControl>().point += 100;
                    hit.collider.gameObject.GetComponent<AttackedByPlayer>().HitZombie(damage, direction, mainCamTransform.position);
                }
                break;
            default:
                break;
            ...
            {//dont allow reloading if gun is lowered while climbing, swimming, or holding object
                ...
                {//if player has no ammo in their inventory for this weapon, do not proceed with reload
                    ...
                    if (bulletsToReload == bulletsPerClip)
                    {
                        ...
                        if (!PistolSprintAnim)
                        {
                            if (weaponNumber == 3|| weaponNumber == 4)
                            {
                                ...
                                Camera.main.GetComponent<Animation>()["CameraReloadAK47"].speed = 1.0f;
                            ...
                            else
                            {
                                ...
                                Camera.main.GetComponent<Animation>()["CameraReloadMP5"].speed = 1.0f;
                        ...
                        weaponMeshAnimation.GetComponent<Animation>().Rewind("Reload");
                        weaponMeshAnimation.GetComponent<Animation>()["Reload"].speed = reloadAnimSpeed;
                        weaponMeshAnimation.GetComponent<Animation>().CrossFade("Reload", 0.35f, PlayMode.StopAll);//play reloading animation
                    }

                    //set reloading var in ironsights script to true
                    shootSight.reloading = true;
                    ...
                    {//If we are reloading weapon one bullet at a time (or bulletsToReload is less than the magazine amount) run code below
                     //determine if bulletsToReload var needs to be changed based on how many bullets need to be loaded						
                        ...
                        {
                            shootSight.reloading = false;//if magazine is full, set reloading var in ironsights script to false
                            bulletsReloaded = 0;
                        ...
                        {//if reloading last round, play normal reloading sound and also chambering effect
                            ...
                            weaponMeshAnimation.GetComponent<Animation>()["Reload"].speed = 1.0f;
                        ...
                        else
                        {
                            ...
                            weaponMeshAnimation.GetComponent<Animation>()["Reload"].speed = shellRldAnimSpeed;
    ...
    public void weaponReady()
    {
            ...
            otherfx.PlayOneShot(otherfx.clip, 1.0f / otherfx.volume);//仅仅播放一次
        ...
        weaponMeshAnimation.GetComponent<Animation>()["Ready"].speed = readyAnimSpeed;
         weaponMeshAnimation.GetComponent<Animation>().CrossFade("Ready", 0.35f, PlayMode.StopAll);
    }

    public bool haveAmmoRoom()
    {//是否还有弹药空余
        if (ammo < maxAmmo)
            return true;
        else
