...
class CharacterMotorMovement {
	...
	var maxForwardSpeed : float = 10.0;//前进后退和左右移动速度
	...
	@System.NonSerialized
	var velocity : Vector3;//在这里声明的velocity变量
...
var movement : CharacterMotorMovement = CharacterMotorMovement();//movement是一个类对象类型
...
private function UpdateFunction () {
	...
	//用来控制初始速度
	var velocity : Vector3 = movement.velocity;//实际的方向由movement中的velocity来控制（Vector3类型）
...
private function ApplyInputVelocityChange (velocity : Vector3) {	
	...
	if (grounded && TooSteep()) {//如果在地面上但是太滑则应用这个函数
		// The direction we're sliding in
		desiredVelocity = Vector3(groundNormal.x, 0, groundNormal.z).normalized;
		// Find the input movement direction projected onto the sliding direction
		var projectedMoveDir = Vector3.Project(inputMoveDirection, desiredVelocity);
		// Add the sliding direction, the spped control, and the sideways control vectors
		desiredVelocity += projectedMoveDir * sliding.speedControl + (inputMoveDirection - projectedMoveDir) * sliding.sidewaysControl;
		...
		desiredVelocity = GetDesiredHorizontalVelocity();//实际计算产生位移的函数
	...
	var velocityChangeVector : Vector3 = (desiredVelocity - velocity);//而velocityChangeVector则本质上来源于desiredVelocity
	...
	//这个用于实际产生位移，所以位移信息来源于velocityChangeVector
...
private function GetDesiredHorizontalVelocity () {
	// Find desired velocity
	//参数是一个Vector3类型的参数
	var currentMoveDirection=movement.camera.transform.rotation*inputMoveDirection;
	var desiredLocalDirection : Vector3 =tr.InverseTransformDirection(currentMoveDirection);
